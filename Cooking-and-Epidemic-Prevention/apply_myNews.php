...
<html>
  ...
  <body style="background-color:#F3EBE0">
    ...
    <p align="center" style="padding-top:150px;"><h1 align="center" style="font-weight:bold">申請人數</h1></p>
	...
	<?php
      ...
      $sql = "SELECT * FROM applys WHERE reply_jNo = '$jNo'";
      ...
      while ($row = mysqli_fetch_assoc($result) and $j <= $records_per_page)
      {
		...
		$mNo = $row["reply_mNo"];
		$sql2 = "SELECT mEmail,mNickname,mPhoto_filename FROM member WHERE mNo=".$mNo;
		$result2 = execute_sql($link, "db_c108193104", $sql2);
		$row2 = mysqli_fetch_assoc($result2);
		$file_name = $row2["mPhoto_filename"];
		  
		echo "<table width='800' align='center' cellspacing='3' cellpadding='8'>";
        echo "<tr>";
        echo "<td width='120' align='center' rowspan='2' style='background-color:#8F7B69'><img src='Thumbnail/headshot/$file_name' style='border-width:1px'></td>";
        
		echo "<td bgcolor='" . $bg1[($j - 1)%5] . "' align='center'  style='padding:15px;font-weight:bold'></td></tr>";
		echo "<tr>";
		echo "<td bgcolor='" . $bg2[($j - 1)%5] . "' style='padding:8px'>暱稱：" . $row2["mNickname"] . "<br>";
        echo "配對日期：" . $row["aDate"] . "<br>";
		echo "配對時間：" . $row["aTime"] . "<br>";
		echo "聯絡方式：" . $row["aContact"] . "<br>";
		echo "電子信箱：" . $row2["mEmail"] . "<br>";
	  ...
	  ?>
		...
		<td align="right" style="padding-top:10px" >
			<button id="btn2" value="add_data" onclick="location.href='show_myNews.php?id=<?php echo $jNo; ?>'">返回</button>
		</td>
		</tr>
		</table>
  <?php
