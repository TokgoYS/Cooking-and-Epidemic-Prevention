<?php
  ...
  else
  {
    ...
    $sql = "SELECT * FROM journal WHERE jNo =".$jNo;
...
<html>
  ...
  <body style="background-color:#F3EBE0">
    ...
    <form name="myForm" method="post" action="update_myNews.php">
      <table align="center" width="800px">
	  <tr align='center'><td bgcolor='#8F7B69' colspan='2' style='padding:20px'></td></tr>
	  
	  <tr>
		<td align="center" colspan="2"><h1 style="font-weight:bold">修改刊登</h1></td>
	  </tr>
		<tr> 
			<td style="height:20px"></td>
		</tr>
		<tr> 
          <td align="right">刊登時間：</td>
          <td><?php echo $row["jDate"] ?></td>
        </tr>
		<tr> 
          <td align="right">教學者：</td>
          <td><?php echo $row2["mNickname"] ?></td>
        </tr>
        <tr> 
          <td align="right">料理名稱：</td>
          <td><input type="text" name="dish" size="8" value="<?php echo $row["jDish"] ?>"></td>
        </tr>
		<tr > 
		...
		<tr > 
          ...
          <td><textarea name="stuff" cols="50" rows="4"><?php echo $row["jStuff"]?></textarea></td>
        </tr>
        <tr > 
          <td align="right">備註說明：</td>
		  <td><textarea name="remarks" cols="50" rows="4"><?php echo $row["jRemarks"]?></textarea></td>
		</tr>
        <tr align="center"> 
          <td colspan="2" align="center" style="padding:15px"> 
            <input id="btn1" type="submit" value="修改資料" onClick="check_data()">
            <input id="btn2" type="button" value="返回" onclick="location.href='show_myNews.php?id=<?php echo $jNo; ?>'">
          </td>
        </tr>
      </table>
    </form>
